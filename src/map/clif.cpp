...
static int clif_send_sub(struct block_list *bl, va_list ap)
{
	...
	switch(type) {
	case AREA:
		if( RBUFW(buf,0) == 0x01c8 && (mapdata->flag[MF_GVG] || mapdata->flag[MF_BATTLEGROUND]) && bl != src_bl && sd->state.packet_filter&2 )
			return 0; // Ignore other player's item usage
		if (sd->state.packet_filter & 8) {

			if (RBUFW(buf, 0) == 0x0983) {
				switch (RBUFW(buf, 2)) {
				case EFST_SMA_READY:
				case EFST_SOULLINK:
				case EFST_SKE:
	...
	case AREA_WOS:
		if (bl == src_bl)
			return 0;
		if (RBUFW(buf, 0) == 0x9fe) {
			len = clif_set_unit_idle(src_bl, buf, true, sd->state.packet_filter & 8);
			clif_send(buf, len, bl, SELF);
			return 0;
		}
		else if (RBUFW(buf, 0) == 0x9fd) {
			struct unit_data* ud = unit_bl2ud(src_bl);
			len = clif_set_unit_walking(src_bl, ud, buf, sd->state.packet_filter & 8);
			clif_send(buf, len, bl, SELF);
			return 0;
		}
	break;
	case AREA_WOC:
		if (sd->chatID || bl == src_bl)
			return 0;
		if( RBUFW(buf,0) == 0x8d && (mapdata->flag[MF_GVG] || mapdata->flag[MF_BATTLEGROUND]) && sd->state.packet_filter&1 )
			return 0;
	break;
	case AREA_WOSC:
	{
...
int clif_send(const uint8* buf, int len, struct block_list* bl, enum send_target type)
{
	...
	case PARTY_BUFF_INFO: //OldTimesRO
		...
		if (p) {
			for(i=0;i<MAX_PARTY;i++){
				...
				if( type != PARTY_BUFF_INFO && type != PARTY && type != PARTY_WOS && bl->m != sd->bl.m ) //OldTimesRO
					continue;

				if( (type == PARTY_AREA || type == PARTY_AREA_WOS) && (sd->bl.x < x0 || sd->bl.y < y0 || sd->bl.x > x1 || sd->bl.y > y1) )
					continue;

				if( type == PARTY_BUFF_INFO && !sd->state.spb ) //OldTimesRO
					continue;
 ...
 *------------------------------------------*/
static int clif_set_unit_idle(struct block_list* bl, unsigned char* buffer, bool spawn, bool hideOpt3)
{
	struct map_session_data* sd;
...
#if PACKETVER >= 20091103
	WBUFL(buf,42) = (sc && !hideOpt3)? sc->opt3 : 0;
	offset+=2;
	buf = WBUFP(buffer,offset);
#elif PACKETVER >= 7
	if (!type) {
		...
		WBUFW(buf,42) = (sc && !hideOpt3)? sc->opt3 : 0;
#else
	WBUFW(buf,42) = (sc && !hideOpt3)? sc->opt3 : 0;
#endif
 ...
 *------------------------------------------*/
static int clif_set_unit_walking(struct block_list* bl, struct unit_data* ud, unsigned char* buffer, bool hideOpt3)
{
	struct map_session_data* sd;
	struct status_change* sc = status_get_sc(bl);
	struct view_data* vd = status_get_viewdata(bl);
...
#if PACKETVER < 7
	WBUFW(buf,46) = (sc && !hideOpt3)? sc->opt3 : 0;
#else
	WBUFL(buf,46) = (sc && !hideOpt3)? sc->opt3 : 0;
	offset+=2; //Shift the rest of elements by 2 bytes.
	buf = WBUFP(buffer,offset);
#endif
...
int clif_spawn(struct block_list *bl)
{
	...
	**/
	...
	len = clif_set_unit_idle(bl, buf,true, false);
...
void clif_getareachar_unit(struct map_session_data* sd,struct block_list *bl)
{
	...
	**/
	...
	ud = unit_bl2ud(bl);
	len = ( ud && ud->walktimer != INVALID_TIMER ) ? clif_set_unit_walking(bl,ud,buf, sd->state.packet_filter & 8) : clif_set_unit_idle(bl,buf,false, sd->state.packet_filter & 8);
