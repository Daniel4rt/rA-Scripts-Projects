...
uint8 pc_isequip(struct map_session_data *sd,int n)
{
	...
	if (sd->sc.count) {
		...
		if (sd->sc.data[SC_SPIRIT] && sd->sc.data[SC_SPIRIT]->val2 == SL_SUPERNOVICE) {
			...
			if (sd->status.base_level > 96 && item->equip & EQP_ARMS && item->type == IT_WEAPON) //OldTimesRO
...
static void pc_bonus_addeff(std::vector<s_addeffect> &effect, enum sc_type sc, short rate, short arrow_rate, unsigned char flag, unsigned int duration)
{
	...
	if (!(flag&(ATF_TARGET | ATF_SELF)))//ATF_AREA: [DanielArt]
...
void pc_bonus2(struct map_session_data *sd,int type,int type2,int val)
{
	...
	case SP_DROP_ADDCLASS: // bonus2 bDropAddClass,c,x;
		...
		break;
	case SP_LONG_ATK_ADD_RACE: // bonus2 bLongAtkAddRace,r,x;
		PC_BONUS_CHK_RACE(type2,SP_LONG_ATK_ADD_RACE);
		if(!sd->state.lr_flag != 2)
			sd->long_atk_add_race[type2] += val;
		break;
	default:
		if (running_npc_stat_calc_event) {
...
void pc_battle_info(struct map_session_data *tsd, struct map_session_data *ssd, uint16 skill_id) //Battleinfo
{
	char output[256];
...
if( (map_getmapflag(sd->bl.m, MF_GVG_CASTLE) || map_getmapflag(sd->bl.m, MF_GVG_TE_CASTLE)) && ((md->mob_id >= 1324 && md->mob_id <= 1363) || (md->mob_id >= 1938 && md->mob_id <= 1946)) && (g = guild_search(sd->status.guild_id)) && (gc = guild_mapindex2gc(map[sd->bl.m].index)) )
{
...
int pc_dead(struct map_session_data *sd,struct block_list *src, uint16 skill_id)
{
	...
	if (src && src->type == BL_PC) {
		...
		pc_battle_info(sd, ssd, skill_id);//Battleinfo
...
void pc_attendance_claim_reward( struct map_session_data* sd ){
	...
	//Sader Attendance Requirement OldTimesRO
	...
	if (!pc_readreg2(sd, attendance_requirement)) {
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "You need to kill 100 monsters to start earning daily rewards.", false, SELF);
		return;
	}
