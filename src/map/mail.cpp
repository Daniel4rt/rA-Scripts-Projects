...
int mail_openmail(struct map_session_data *sd)
{
	...
	if( sd->state.storage_flag || sd->state.vending || sd->state.buyingstore || sd->state.trading || map_getmapflag(sd->bl.m, MF_GVG_CASTLE) || map_getmapflag(sd->bl.m, MF_BATTLEGROUND) || map_getmapflag(sd->bl.m, MF_PVP))
		return 0;
