...
void run_tomb(struct map_session_data* sd, struct npc_data* nd)
{
	...
	strftime(time, sizeof(time), "%H:%M", newtime);
	snprintf(buffer, sizeof(buffer), msg_txt(sd, 828), time);
	clif_scriptmes(sd, nd->bl.id, buffer);
	}
	...
	clif_scriptmes(sd, nd->bl.id, msg_txt(sd, 829));
	
	snprintf(buffer, sizeof(buffer), msg_txt(sd, 830), nd->u.tomb.killer_name[0] ? nd->u.tomb.killer_name : "[Server]", commash(std::to_string(nd->u.tomb.damage1)).c_str());
	clif_scriptmes(sd, nd->bl.id, buffer);

	//Aglao - Tomb MVP Top Damage - Fix temporal para evitar crash buffer
	/*if (nd->u.tomb.damage2 != 0)
	{
		snprintf(buffer, sizeof(buffer), msg_txt(sd, 831), nd->u.tomb.p2, commash(std::to_string(nd->u.tomb.damage2)).c_str());
		clif_scriptmes(sd, nd->bl.id, buffer);
	}

	if (nd->u.tomb.damage3 != 0)
	{
		snprintf(buffer, sizeof(buffer), msg_txt(sd, 832), nd->u.tomb.p3, commash(std::to_string(nd->u.tomb.damage3)).c_str());
		clif_scriptmes(sd, nd->bl.id, buffer);
	}*/
	//Fin Aglao - Tomb MVP Top Damage - Fix temporal para evitar crash buffer
