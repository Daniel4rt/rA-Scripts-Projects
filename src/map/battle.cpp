...
#define APPLY_CARDFIX(damage, fix) { (damage) = (damage) - (int64)(((damage) * (1000 - (fix))) / 1000); }
	...
	case BF_WEAPON:
		...
		if (sd && !(nk & NK_NO_CARDFIX_ATK) && (left & 2)) {
			...
			if (sd->state.arrow_atk) { // Ranged attack [LongAtkAddRace @DanielArt]
				cardfix = cardfix * (100 + sd->long_atk_add_race[tstatus->race] + sd->right_weapon.addrace[tstatus->race] + sd->arrow_addrace[tstatus->race] +
					sd->long_atk_add_race[RC_ALL] + sd->right_weapon.addrace[RC_ALL] + sd->arrow_addrace[RC_ALL]) / 100;
				if (!(nk & NK_NO_ELEFIX)) { // Affected by Element modifier bonuses
...
int64 battle_calc_damage(struct block_list* src, struct block_list* bl, struct Damage* d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sc && sc->data[SC_MAXPAIN])
		return 0;
	if (skill_id == MO_EXTREMITYFIST) {//Zelos - Da�o de asura limitado a 2M
//		damage = min(damage, 2000000);
		damage = cap_value(damage,0,1000000);
	}
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list* src, struct block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	case SN_SHARPSHOOTING:
	case MA_SHARPSHOOTING:
		if (sd && tsd && ((sd->status.party_id && sd->status.party_id == tsd->status.party_id) || (sd->status.guild_id && sd->status.guild_id == tsd->status.guild_id)))
			break;
		else
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		default: {
			...
			case SL_STUN: //Full damage is dealt on small/medium targets
				...
				break;
			case SL_SMA: // [DanielArt]
				if (src->type != BL_MOB) {
					if (sd->status.base_level < 100) {
						skillratio += -90 + status_get_lv(src) / 2;
						break;
					}
					else {
						skillratio += -40;
...
int battle_check_target(struct block_list* src, struct block_list* target, int flag)
{
	...
	if ( s_bl->type == BL_PC && t_bl->type == BL_MOB ) {
		struct map_session_data *sd = BL_CAST( BL_PC, s_bl );
		if ( ( ( (TBL_MOB*)target )->mob_id == 1288 && !strcmp( mapindex_id2name(sd->mapindex), "scythero" ) ) &&
			( sd->status.guild_id == mapreg_readreg( add_str("$goegid") ) || battle_getcurrentskill(src) > 0 ) )
		return 0;
	}
	if ( s_bl->type == BL_PC && t_bl->type == BL_MOB ) {
		struct map_session_data *sd = BL_CAST( BL_PC, s_bl );
		if ( ( ( (TBL_MOB*)target )->mob_id == 3243 && !strcmp( mapindex_id2name(sd->mapindex), "league" ) ) &&
			( sd->status.guild_id == mapreg_readreg( add_str("$doegid") ) || battle_getcurrentskill(src) > 0 ) )
		return 0;
	}
	if ( s_bl->type == BL_PC && t_bl->type == BL_MOB ) {
		struct map_session_data *sd = BL_CAST( BL_PC, s_bl );
		if ( ( ( (TBL_MOB*)target )->mob_id == 1288 && !strcmp( mapindex_id2name(sd->mapindex), "soul" ) ) &&
			( sd->status.guild_id == mapreg_readreg( add_str("$roegid") ) || battle_getcurrentskill(src) > 0 ) )
		return 0;
	}
	if ( s_bl->type == BL_PC && t_bl->type == BL_MOB ) {
		struct map_session_data *sd = BL_CAST( BL_PC, s_bl );
		if ( ( ( (TBL_MOB*)target )->mob_id == 3255 && !strcmp( mapindex_id2name(sd->mapindex), "training" ) ) &&
			( sd->status.guild_id == mapreg_readreg( add_str("$toegid") ) || battle_getcurrentskill(src) > 0 ) )
		return 0;
	}
